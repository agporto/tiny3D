# tiny3d_sycl_target_sources(name ...)
#
...
#
# Example usage:
#   tiny3d_sycl_target_sources(core PRIVATE a.cpp b.cpp)
#   tiny3d_sycl_target_sources(core PUBLIC a.cpp b.cpp)
#   tiny3d_sycl_target_sources(core VERBOSE PRIVATE a.cpp b.cpp)
#   tiny3d_sycl_target_sources(core VERBOSE PUBLIC a.cpp b.cpp)
function(tiny3d_sycl_target_sources target)
    cmake_parse_arguments(arg "PUBLIC;PRIVATE;INTERFACE;VERBOSE" "" "" ${ARGN})
    if(arg_UNPARSED_ARGUMENTS)
        if(arg_PUBLIC)
            target_sources(${target} PUBLIC ${arg_UNPARSED_ARGUMENTS})
            message(STATUS "tiny3d_sycl_target_sources(${target}): PUBLIC")
        elseif (arg_PRIVATE)
            target_sources(${target} PRIVATE ${arg_UNPARSED_ARGUMENTS})
            message(STATUS "tiny3d_sycl_target_sources(${target}): PRIVATE")
        elseif (arg_INTERFACE)
            target_sources(${target} INTERFACE ${arg_UNPARSED_ARGUMENTS})
            message(STATUS "tiny3d_sycl_target_sources(${target}): INTERFACE")
        else()
            message(FATAL_ERROR "Invalid syntax: tiny3d_sycl_target_sources(${name} ${ARGN})")
        endif()

        if(BUILD_SYCL_MODULE)
            foreach(sycl_file IN LISTS arg_UNPARSED_ARGUMENTS)
                set_source_files_properties(${sycl_file} PROPERTIES
                    COMPILE_OPTIONS "-fsycl;-fsycl-targets=${TINY3D_SYCL_TARGETS}")
                if(arg_VERBOSE)
                    message(STATUS "tiny3d_sycl_target_sources(${target}): marked ${sycl_file} as SYCL code")
                endif()
            endforeach()
        endif()
    endif()
